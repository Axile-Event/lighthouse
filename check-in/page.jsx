...
export default function LighthouseCheckInPage() {
  ...
  const eventOptions = events.map((ev) => ({
    ...
    label: ev.event_name || ev.name || ev.title,
    icon: Ticket
  }));
      ...
      if (
        ...
        try {
          ...
          if (
            ...
            toast.error(
              "Camera permission denied. Please refresh and allow access.",
              { duration: 6000 }
            );
          } else if (
            ...
            toast.error(
              "Camera is already in use. Please close other apps and try again."
            );
          } else {
            toast.error(
  ...
  const startScanning = async () => {
    ...
    if (
      ...
      toast.error(
        "Camera requires HTTPS or localhost. Current site is not secure.",
        { duration: 8000 }
      );
      return;
    }
    ...
    try {
      ...
      if (
        ...
        toast.error(
          "Camera permission denied. Check system and browser settings, then refresh.",
          { duration: 10000 }
        );
      } else if (
        err.name === "NotFoundError" ||
        ...
        toast.error(
          "Camera in use by another app. Close other apps and refresh."
        );
      } else {
  ...
  const processCheckIn = async (rawCode, isManual = false) => {
    ...
    try {
      const response = await api.post("/api/admin/tickets/check-in/", {
        ticket_id: ticketId,
      ...
      const attendeeName =
        data.ticket?.student_full_name || "Attendee";
        ...
        <main className="grid grid-cols-1 lg:grid-cols-12 gap-8">
                    ...
                    {!isScanning ? (
                      <div className="text-center space-y-6 px-10">
                        ...
                        <div className="space-y-2">
                          <h3 className="text-xl font-bold italic tracking-wide">
                            Ready to Verify?
                          </h3>
                          <p className="text-gray-500 max-w-xs text-sm">
                            Start the camera to scan attendee QR codes.
                          </p>
                        </div>
                        <button
          ...
          <aside className="lg:col-span-4 flex flex-col h-full bg-[#0A0A0A] border border-white/5 rounded-[2.5rem] overflow-hidden shadow-xl min-h-[500px]">
            <div className="p-8 border-b border-white/5 flex items-center justify-between bg-gradient-to-br from-rose-500/5 to-transparent">
              ...
              <span className="bg-rose-500/10 text-rose-500 text-[10px] font-black px-3 py-1 rounded-full">
                {history.length}
              </span>
            </div>
